...
**참여 인원**: 4명  
**핵심 목표**: MVC 기반의 동적 콘텐츠 렌더링 및 사용자 편의성을 고려한 인터페이스 설계  
...
- **Global Style 정의**: 프로젝트 전반의 폰트, 컬러 시스템, 여백 등을 규격화해서 사용자에게 시각적 통일성을 주고자 노력했습니다.

### 사용자 인증 및 회원 관리 시스템
- **Flow 설계**: 회원가입부터 로그인, 아이디/비밀번호 찾기까지 이어지는 사용자 흐름을 설계했습니다.
- **개인화 인터페이스**: 마이페이지를 통해 사용자 본인의 정보를 관리할 수 있는 대시보드 형태의 UI를 구현했습니다.
- **API 통합**: Kakao Login API를 연동해서 접근성을 향상시켰습니다.

### 관리자용 통합 관리 페이지
- **회원 목록 관리**: 전체 회원 정보를 한눈에 확인하고 관리할 수 있는 전용 페이지를 구축했습니다.
- **조회 편의성**: 많은 양의 회원 데이터를 표 형태로 정돈해서 출력함으로써 관리자가 정보를 식별하기 좋게 구성했습니다.
